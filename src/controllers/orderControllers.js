...
export const handleNewOrder = TryCatch(async (req, res, next) => {
    ...
    const allowedFields = validateAllowedFields(req, newOrderRequestBody);
    if (allowedFields) {
...
export const handlerMyOrders = TryCatch(async (req, res, next) => {
    const { id } = req.query;
    const allowedQueryParams = ["id"];
    const invalidQuery = validateAllowedQueryParams(req, allowedQueryParams);
    if (invalidQuery) {
        return next(new ErrorHandler(`Invalid query params: ${invalidQuery.join(", ")}`, 400));
    }
    let orders = [];
    if (nodeCache.has(`my_orders_${id}`)) {
        orders = JSON.parse(nodeCache.get(`my_orders_${id}`));
    }
    else {
...
export const handleGetAllOrders = TryCatch(async (req, res, next) => {
    let orders = [];
    if (nodeCache.has("all-admin-orders")) {
        orders = JSON.parse(nodeCache.get("all-admin-orders"));
    }
    else {

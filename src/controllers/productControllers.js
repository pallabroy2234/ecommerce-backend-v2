...
export const handleNewProduct = TryCatch(async (req, res, next) => {
    ...
    const allowedFields = ["name", "category", "price", "stock", "image"];
...
export const handleGetAllCategories = TryCatch(async (req, res, next) => {
...
export const handleGetAllAdminProducts = TryCatch(async (req, res, next) => {
    let products = [];
...
export const handleGetSingleProduct = TryCatch(async (req, res, next) => {
    const { id } = req.params;
    let product;
...
const prepareUpdates = (body, allowedFields) => {
    let updates = {};
    for (let key in body) {
...
export const handleUpdateSingleProduct = TryCatch(async (req, res, next) => {
    const { id } = req.params;
    const image = req.file;
    const productExists = await Product.findById({ _id: id });
    if (!productExists)
        return next(new ErrorHandler("Product not found", 404));
    const allowFields = ["name", "category", "price", "stock", "image"];
    // * Update allowed fields
    let updates = prepareUpdates(req.body, allowFields);
...
export const handleGetAllProducts = TryCatch(async (req, res, next) => {
    ...
    const allowQueries = ["search", "price", "category", "sort", "page"];
    ...
    const baseQuery = {};
    if (search)
        baseQuery.name = { $regex: searchPattern };
    if (price)

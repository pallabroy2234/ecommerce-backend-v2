...
// * Order Processing
export const orderProcessing = async (orderItems) => {
    try {
        ...
        const duplicates = allOrderProductId.filter((id, index, self) => self.indexOf(id) !== index);
        if (duplicates.length > 0) {
            const duplicateNames = [];
            for (let id of duplicates) {
                const product = await Product.findById({ _id: id }).select("name");
                if (product) {
                    duplicateNames.push(product.name);
                }
            }
            return new ErrorHandler(`Order contains duplicate product(s): ${duplicateNames.join(", ")}`, 400);
        }
        ...
        for (let order of orderItems) {
            ...
            if (!product) {
                return new ErrorHandler(`Product not found with id: ${order.productId}`, 404);
            }
            if (product.stock < order.quantity) {
                return new ErrorHandler(`Product : ${product.name} is out of stock`, 400);
            }
            // Reduce the stock
...
export const validateShippingInfoFields = (req) => {
    const allowedFields = [
        "address",
        "country",
        "city",
    ...
    if (invalidFields.length > 0) {
        return new ErrorHandler(`Invalid shipping info field: ${invalidFields.join(", ")}`, 400);
    }
    return null;
};
// * validate orderItems allowed fields using Set Method and return error if invalid fields are return all invalid fields
export const validateOrderItemsFields = (req) => {
    let allKeys = new Set();
    let allowedKeys = new Set(["productId", "quantity"]);
    let invalid = new Set();
    ...
    if (invalid.size > 0) {
        return new ErrorHandler(`Invalid order items field: ${[...invalid].join(", ")}`, 400);
    }
    return null;
};

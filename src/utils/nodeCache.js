...
export const invalidateCache = ({ product, order, admin, coupon, couponId, userId, orderId, productId, }) => {
    try {
        if (product) {
            const productsKeys = ["admin-products", "latestProducts", "categories"];
        ...
        if (order) {
            const orderKeys = ["all-admin-orders", `my_orders_${userId}`, `order-${orderId}`];
